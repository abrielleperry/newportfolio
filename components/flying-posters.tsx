...
void main() {
  ...
  // Use the same UV coordinates regardless of which side is facing the camera
  // This ensures the same image appears on both sides
  vec2 uv = vUv * scale + (1.0 - scale) * 0.5;
  
  // Flip the UVs horizontally for the back side to maintain correct orientation
  if (vNormal.z < 0.0) {
    uv.x = 1.0 - uv.x;
  }
...
class Media {
  ...
  createShader() {
    ...
    this.program = new Program(this.gl, {
      ...
      cullFace: false, // Disable face culling to render both sides
    });
...
class Canvas {
  ...
  constructor({
    ...
    // Preload textures to avoid glitching
  ...
  createRenderer() {
    ...
    // Enable depth testing for proper rendering of both sides
    this.gl.enable(this.gl.DEPTH_TEST);
    // Disable face culling to render both sides of the planes
  ...
  preloadTextures() {
    // Create a single loader for all textures
    const textures = this.items.map((src) => {
    ...
    // Store textures to prevent garbage collection
